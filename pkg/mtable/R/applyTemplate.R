...
formatVec <- function(x,formats,default="f",digits=min(3,getOption("digits")),
  ...
  res <- sapply(i,function(i) if(length(formats[[i]]))
      sapply(formats[[i]],function(format)formatOne(x,spec=format,format=default,digits=digits,signif.symbols=signif.symbols))
      else NULL
      )
}
...
applyTemplate <- function(x,template,float.style=getOption("float.style"),
   ...
   for(i in 1:length(patterns)){
      ...
      formatted <- formatVec(x[i],formats,default=float.style,digits=digits,signif.symbols=signif.symbols)
   ...
   if(length(npatterns)){
      for(n in names(npatterns)){
          ...
          positions <- attr(targets,"positions")
          formats <- lapply(targets,get.format)
          formatted <- formatVec(x[n],formats,default=float.style,digits=digits,signif.symbols=signif.symbols)
          for(j in 1:length(template)){
            if(length(targets[[j]])){
              targets.j <- targets[[j]]
              formatted.j <- formatted[[j]]
