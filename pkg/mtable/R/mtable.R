...
mtable <- function(...,
  ...
  relabel.arg.present <- !missing(relabel)
  getCoef1 <- function(coef,contrasts,xlevels){
        if(relabel.arg.present){
          if(!is.list(relabel)) stop("list expected as 'relabel' argument")
          for(d in seq_along(dim(coef))){
            relabel.arg <- c(list(coef,dim=d),relabel)
...
#   coefs <- lapply(seq(n.args),getEstimates,coefs,contr,xlev)
  ...
  if(isTRUE(summary.stats) || length(summary.stats)) {
    ...
    if(isTRUE(summary.stats)){
      ...
      summary.stats <- lapply(stemplates,names)
      ...
      for(i in 1:length(args)){
        ...
        tmp.sumstat <- getDefaultSumStat(args[[i]])
        if(length(tmp.sumstat))
          summary.stats[[i]] <- tmp.sumstat
      }
