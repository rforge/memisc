spss.system.file <- function(
    ...
    print.format.type <- sapply(data.spec$variables,"[[","print")[3,]
    tmp <- print.format.type %in% c(20,22:24,26:30,38,39)
    is.date.time <- logical(length(variables))
    is.date.time[types>=0] <- tmp

    variables[types==0 & !is.date.time] <- list(new("double.item"))
    variables[types==0 & is.date.time] <- list(new("datetime.item",origin="1582-10-14",tzone="GMT"))
...
setMethod("rewind","spss.system.importer",function(x,...){
...
setMethod("path","spss.system.importer",function(x,...) attr(x@ptr,"file.name"))